--[[
	...
	Developed by Chillz
	Translated into Simplified Chinese by FOCUS (For personal use only.)
...
local function main()
	...
	Explorer.InitRightClick = function()
		...
		context:Register("DUPLICATE",{Name = "克隆", IconMap = Explorer.MiscIcons, Icon = "Copy", DisabledIcon = "Copy_Disabled", Shortcut = "Ctrl+D", OnClick = function()
			local clone = game.Clone
			local sList = selection.List
			local newSelection = {}
			local count = 1
		...
		local OldAnimation
		context:Register("PLAY_TWEEN",{Name = "播放渐变动画", IconMap = Explorer.MiscIcons, Icon = "Play", OnClick = function()
			local sList = selection.List
		...
		context:Register("BLOCK_REMOTE",{Name = "阻止传输", IconMap = Explorer.MiscIcons, Icon = "Delete", OnClick = function()
			local sList = selection.List
		...
		context:Register("UNBLOCK_REMOTE",{Name = "解除阻止", IconMap = Explorer.MiscIcons, Icon = "Play", OnClick = function()
			local sList = selection.List
			for i, list in sList do
				local obj = list.Obj
		...
		context:Register("COPY_API_PAGE",{Name = "复制Roblox API页面URL", IconMap = Explorer.MiscIcons, Icon = "Reference", OnClick = function()
			local sList = selection.List
			if #sList == 1 then
				env.setclipboard(
		...
		context:Register("SPECTATE_OBJECT",{Name = "观察对象 (右键可重置)", IconMap = Explorer.ClassIcons, Icon = "Camera", OnClick = function()
			local sList = selection.List
			local isa = game.IsA
		...
		context:Register("VIEW_MODEL",{Name = "浏览模型", IconMap = Explorer.ClassIcons, Icon = "Camera", OnClick = function()
			local sList = selection.List
			local isa = game.IsA
		...
		context:Register("VIEW_SCRIPT",{Name = "浏览脚本", IconMap = Explorer.MiscIcons, Icon = "ViewScript", OnClick = function()
			local scr = selection.List[1] and selection.List[1].Obj
			if scr then ScriptViewer.ViewScript(scr) end
		end})
		context:Register("DUMP_FUNCTIONS",{Name = "转储函数", IconMap = Explorer.MiscIcons, Icon = "ViewScript", OnClick = function()
			local scr = selection.List[1] and selection.List[1].Obj
			if scr then ScriptViewer.DumpFunctions(scr) end
		end})

		context:Register("FIRE_TOUCHTRANSMITTER",{Name = "信号触摸发射器", OnClick = function()
		...
		context:Register("FIRE_CLICKDETECTOR",{Name = "信号单击检测器", OnClick = function()
			local hrp = plr.Character and plr.Character:FindFirstChild("HumanoidRootPart")
			if not hrp then return end
			for _, v in ipairs(selection.List) do if v.Obj and v.Obj:IsA("ClickDetector") then fireclickdetector(v.Obj) end end
		end})

		context:Register("FIRE_PROXIMITYPROMPT",{Name = "信号接近提示", OnClick = function()
			local hrp = plr.Character and plr.Character:FindFirstChild("HumanoidRootPart")
			if not hrp then return end
		...
		context:Register("VIEW_PLAYER",{Name = "浏览玩家", IconMap = Explorer.ClassIcons, Icon = 5, OnClick = function()
			local newSelection = {}
			local count = 1
			local sList = selection.List
			local isa = game.IsA
		...
		context:Register("REFRESH_NIL",{Name = "刷新Nil实例", OnClick = function()
			Explorer.RefreshNilInstances()
		end})

		context:Register("HIDE_NIL",{Name = "隐藏Nil实例", OnClick = function()
...
return search]==]
	...
	Explorer.Init = function()
		...
		local explorerItems = create({
			...
			{4,"TextBox",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,ClearTextOnFocus=false,Font=3,Name="SearchBox",Parent={3},PlaceholderColor3=Color3.new(0.39215689897537,0.39215689897537,0.39215689897537),PlaceholderText="搜索工作区 workspace",Position=UDim2.new(0,4,0,0),Size=UDim2.new(1,-24,0,18),Text="",TextColor3=Color3.new(1,1,1),TextSize=14,TextXAlignment=0,}},
	...
	Lib.ContextMenu = (function()
		...
		local function createGui(self)
			local contextGui = create({
				...
				{14,"TextLabel",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,BorderSizePixel=0,Font=3,Name="EntryName",Parent={13},Position=UDim2.new(0,24,0,0),Size=UDim2.new(1,-24,1,0),Text="克隆",TextColor3=Color3.new(0.86274516582489,0.86274516582489,0.86274516582489),TextSize=14,TextXAlignment=0,}},
			...
			colorBox.InputBegan:Connect(function(input)
				if (input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch) then
					...
					if not editor then
						...
						editor.Window:SetTitle("ColorSequence 颜色选择器")
...
local function main()
	...
	ModelViewer.ViewModel = function(item, updating)
		...
		if not updating then
			...
			window:SetTitle(item.Name.." - 模型查看器")
	...
	ModelViewer.Init = function()
		...
		context:Register("SAVE_INST",{Name = "保存到文件", OnClick = function()
			if model then
				window:SetTitle(originalModel.Name.." - 模型查看器 - 保存中")
				...
				if success then
					window:SetTitle(originalModel.Name.." - 模型查看器 - 已保存")
					context:Hide()
					task.wait(5)
					if model then
						window:SetTitle(originalModel.Name.." - 模型查看器")
					end
				else
					window:SetTitle(originalModel.Name.." - 模型查看器 - 错误")
...
local function main()
	...
	Properties.InitEntryStuff = function()
		Properties.EntryTemplate = create({
			...
			{3,"TextLabel",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,Font=3,Name="PropName",Parent={2},Position=UDim2.new(0,2,0,0),Size=UDim2.new(1,-2,1,0),Text="锚固",TextColor3=Color3.new(1,1,1),TextSize=14,TextTransparency=0.10000000149012,TextTruncate=1,TextXAlignment=0,}},
	...
	Properties.Init = function() -- TODO: MAKE BETTER
		local guiItems = create({
			...
			{2,"Frame",{BackgroundColor3=Color3.new(0.20392157137394,0.20392157137394,0.20392157137394),BorderSizePixel=0,Name="ToolBar",Parent={1},Size=UDim2.new(1,0,0,22),}},
			{3,"Frame",{BackgroundColor3=Color3.new(0.14901961386204,0.14901961386204,0.14901961386204),BorderColor3=Color3.new(0.1176470592618,0.1176470592618,0.1176470592618),BorderSizePixel=0,Name="SearchFrame",Parent={2},Position=UDim2.new(0,3,0,1),Size=UDim2.new(1,-6,0,18),}},
			{4,"TextBox",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,ClearTextOnFocus=false,Font=3,Name="SearchBox",Parent={3},PlaceholderColor3=Color3.new(0.39215689897537,0.39215689897537,0.39215689897537),PlaceholderText="搜索特性",Position=UDim2.new(0,4,0,0),Size=UDim2.new(1,-24,0,18),Text="",TextColor3=Color3.new(1,1,1),TextSize=14,TextXAlignment=0,}},
			{5,"UICorner",{CornerRadius=UDim.new(0,2),Parent={3},}},
...
local function main()
	...
	SaveInstance.Init = function()
		...
		local decompileThread = AddTextbox("反编译器最大线程数", "3", 15)
		...
		local NilObj = AddCheckbox("保存Nil实例", SaveInstanceArgs.NilInstances)
		...
		local IsolateStarterPlr = AddCheckbox("隔离 StarterPlayer", SaveInstanceArgs.IsolateStarterPlayer)
		IsolateStarterPlr.OnInput:Connect(function()
			SaveInstanceArgs.IsolateStarterPlayer = IsolateStarterPlr.Toggled
		end)
		
		local IgnoreDefaultProps = AddCheckbox("忽略默认特性", SaveInstanceArgs.IgnoreDefaultProps)
	...
	ScriptViewer.ViewScript = function(scr)
		...
		if not s or not source then
			...
			if scr:IsA("Script") and scr.RunContext == Enum.RunContext.Legacy and not scr:IsA("LocalScript") then
				source = source .. "-- 原因: 脚本可能正在服务器上运行, 或者你的执行器不支持反编译器.\n"
			else
				source = source .. "-- 原因: 你的执行器不支持反编译器.\n"
			end
			source = source .. "-- 执行器: "..executorName.." ("..executorVersion..")"
		else
			PreviousScr = scr
			local decompiled = source

			source = "-- 脚本路径: "..getPath(scr).."\n"
			source = source .. "-- 反编译共需要 "..tostring(math.floor( (tick() - oldtick) * 100) / 100).."s.\n"
			source = source .. "-- 执行器: "..executorName.." ("..executorVersion..")\n\n"
...
local function main()
	...
	SecretServicePanel.Init = function()
		...
		toggle.MouseButton1Click:Connect(function()
			...
			addOutput({
				Text = "控制台 "..(outputOn and "已启用" or "已禁用"),
...
Main = (function()
	...
	Main.CreateIntro = function(initStatus) -- TODO: Must theme and show errors
		local gui = create({
			...
			{6,"TextLabel",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,Font=3,Name="Desc",Parent={3},Position=UDim2.new(0,-230,0,60),Size=UDim2.new(0,180,0,25),Text="终极调试套件 [ZH-CN]",TextColor3=Color3.new(1,1,1),TextSize=18,TextTransparency=1,}},
			...
			{11,"TextLabel",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,Font=3,Name="Creator",Parent={2},Position=UDim2.new(1,-110,1,-20),Size=UDim2.new(0,105,0,20),Text="由Chillz开发, FOCUSED_LIGHT翻译为简体中文 (仅用于私人)",TextColor3=Color3.new(1,1,1),TextSize=14,TextXAlignment=1,}},
	...
	Main.SetMainGuiOpen = function(val)
		...
		Main.MainGui.OpenButton.Text = val and "关闭" or "Dex++ [ZH-CN]"
	...
	Main.CreateMainGui = function()
		...
		Main.CreateApp({Name = "特性", IconMap = Main.LargeIcons, Icon = "Properties", Open = true, Window = Properties.Window})
		...
		Main.CreateApp({Name = "模型查看器", IconMap = Main.LargeIcons, Icon = 6, Window = ModelViewer.Window})
		...
		Main.CreateApp({Name = "秘密服务面板", IconMap = Main.LargeIcons, Icon = "Output", Window = SecretServicePanel.Window})
	...
	Main.Init = function()
		...
		-- Loading bypasses
		intro.SetProgress("正在加载 Adonis绕过",0.1)
		pcall(Main.LoadAdonisBypass)
		
		intro.SetProgress("正在加载 GC绕过",0.2)
		pcall(Main.LoadGCBypass)

		-- Fetch version if needed
		intro.SetProgress("正在获取 Roblox 版本",0.3)
		if Main.Elevated then
		...
		API = Main.FetchAPI(
			function()
				intro.SetProgress("正在获取 API, 请稍候.",0.4)
			end,
			function()
				intro.SetProgress("正在获取 API, 由于 API 文件太大, 请等待下载.",0.45)
			end,
			function()
				intro.SetProgress("正在获取 API, LOL 你不会还在下载吧? 废物WiFi xD",0.475)
			end
		)
		Lib.FastWait()
