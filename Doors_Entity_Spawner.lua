...
local Window = OrionLib:MakeWindow({Name = "FOCUS的人机生成器（持续更新）", HidePremium = false, SaveConfig = true,IntroText = "欢迎使用此生成器", ConfigFolder = "作者dy号：FOCUS_MAX"})
...
local SillyBillies = {"玩的开心!?","@DY Film_A-101 你就是个几八(#电影NMSL#)","Huh? 这做的什么玩意这是? !","饿啊磕到甲沟炎了","666这个入是桂","虽然这不是辅助脚本但我还是希望你能玩下去"}
	message("Modifiers已启动")
	task.wait(2)
	message("脚本作者：FOCUS")
	task.wait(2)
	message("dy号：FOCUS_MAX")
	task.wait(4)
...
game.ReplicatedStorage.GameData.LatestRoom.Changed:Connect(function(v)
	...
	if v == 1  then
		message2('The Hell','rbxassetid://1835274270')
	if v == 49 then
	...
	if v == 60 then
		...
		message2('The Nightmare Depths',Sound)
		Bravo=true
		Beans = true
	if v == 90 then
		message2('The Lonely Garden','rbxassetid://1847269119')
	if v == 100  then
		message2('The Abandoned Electrical Room','rbxassetid://1837449237')
	if v == 150  then
		message2('The Dangerous Nest','rbxassetid://1835274270')
	if v == 160  then
		message2('The Bottomless Sewers','rbxassetid://1835274270')
	if v == 200  then
		message2('The Polluted Dam','rbxassetid://1835274270')
	end
